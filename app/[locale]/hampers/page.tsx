  ...
  return (
    <div className="container-cmm py-14">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {hampers.map((h) => (
          <div key={h.slug} className="group overflow-hidden rounded-2xl border border-brand-pink/10 bg-brand-creamlight">
            ...
            <div className="p-4">
              ...
              <p className="mt-1 text-sm text-brand-pink/70">{h.short}</p>
              {h.price && <p className="mt-2 text-sm font-medium text-brand-pink">{h.price}</p>}
            </div>
          </div>
        ))}
        {extra.map((h) => (
          <div key={h.label} className="group overflow-hidden rounded-2xl border border-brand-pink/10 bg-brand-creamlight">
            <div className="aspect-[4/3] overflow-hidden bg-[#F4EEE2]">
              {/* eslint-disable-next-line @next/next/no-img-element */}
